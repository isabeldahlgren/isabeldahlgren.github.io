...
- If instead $f(n) = d(n)$, the function counting the number of divisors of a positive integer $n$, then a [theorem of Dirichlet](https://mathworld.wolfram.com/DirichletDivisorProblem.html) asserts that 

$$\sum_{n \le x} f(n) = x \log x + (2 \gamma - 1)x + O(\sqrt{x}).$$

- Actually, the [Prime number theorem](https://en.wikipedia.org/wiki/Prime_number_theorem#Proof_sketch) is equivalent to an estimate of the above form. Just take $f$ to be the von Mangoldt function $\Lambda$, defined by
